...
class ColumnInfo {
  ...
  const ColumnInfo([this._varName, this._converterFromSql, this._converterToSql, this._calcExpression,
    this._queryRef, this._relation = ColumnRelation.normal]);
  ...
  Future<void> setterFromSql(InstanceMirror instance, Object sqlValue) async {
    Object value = _queryRef != null ? (await _queryRef.getEntityById(sqlValue)) : _converterFromSql(sqlValue);

    instance.invokeSetter(_varName, value);
  }

  Object getterToSql(InstanceMirror instance) => _queryRef != null
      ? (_queryRef.getIdValueFromEntity( instance.invokeGetter(_varName) ) ) : _converterToSql(instance.invokeGetter(_varName));
...
Future<Map<String, ColumnInfo>> initColumnEntityByTable(String tableName, ClassMirror typeInstance,
    QuerySelectyData selectFunc, DbAbstract db) async {
  ...
  for(final row in infoColumnsTable) {
    ...
    final QueryAbstract query = (converterFromSql == stubEntityConverter)
        ? _getFieldEntityQuery(field.value.reflectedType, db) : null;

    final ConverterSql converterToSql = getToSqlConverter(sqlType, field.value.reflectedType);

    MapEntry<ColumnRelation, String> relationCalc = _getColumnRelationCalc(tableName, columnName, row["pk"].toString(), field.value.metadata);

    final columnInfo = ColumnInfo(field.key, converterFromSql, converterToSql, relationCalc.value, query, relationCalc.key);
...
Map<String, ColumnInfo> addAbsentColumns(Map<String, ColumnInfo> columns, Map<String, dynamic> row,
    ClassMirror typeInstance, DbAbstract db) {
...
bool isAbsentPkColumn(Map<String, ColumnInfo> entityMetaData) => entityMetaData.values
    .firstWhere((it)=> it.relation == ColumnRelation.primaryKey, orElse: ()=> null) == null;
...
MapEntry<ColumnRelation, String> _getColumnRelationCalc(String table, String columnTable,
    String isPkColumn, List<Object> metadata) {

  return _getPrimaryKeyRelation(table, columnTable, isPkColumn, metadata) ??
      _getCalcRelation(metadata);
}

MapEntry<ColumnRelation, String> _getPrimaryKeyRelation(String table, String columnTable,
    String isPkColumn, List<Object> metadata) {
...
String _defaultGenerateId(String idColumn, String table) => "select COALESCE( MAX($idColumn), 0) + 1 MAX_ID from $table";
...
MapEntry<String, VariableMirror> _getFieldByAnnotation(ClassMirror typeInstance, String annotationName) {
  ...
  final annot = typeInstance?.declarations?.entries
      ?.firstWhere((it) =>
  (it?.value is VariableMirror) && (it?.value?.metadata != null) &&
      it?.value?.metadata?.firstWhere((annotation) =>
      annotation is Column &&
          compareColumnByAnnotationName(annotation?.name, annotationName), orElse: () => null) != null
      , orElse: () => null);
...
MapEntry<String, VariableMirror> _getFieldByName(ClassMirror typeInstance, String columnName) {
    ...
    final field = typeInstance?.declarations?.entries?.firstWhere((it) =>
    (it.value is VariableMirror) && compareColumnByFieldName(it.key, columnName) &&
...
String _getIdAnnotation(List<Object> metadataColumn) {
  ...
  return id == null ? null :
  (id.querySequence == null ? "" : id.querySequence.trim());
}
...
String _getCalcAnnotation(List<Object> metadataColumn) {
  ...
  (calc.selectById == null ? "" : calc.selectById.trim());
}
